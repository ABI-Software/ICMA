...
public:
  ...
  void
  CrossProduct(const Vector3D& vec1, const Vector3D& vec2)
  {
  ...
  inline Vector3D
  operator +(const Vector3D& rkVector) const
  {
  ...
  inline Vector3D
  operator *(const Real fScalar) const
  {
...
inline Vector3D
operator*(Real scalar, const Vector3D& rhs)
{
...
inline Vector3D
operator*(const gtMatrix& m, const Vector3D& v) // no translation
{
  Vector3D r;
...
inline Vector3D
CrossProduct(const Coordinate3D& vec1, const Coordinate3D& vec2)
{
...
template<typename V>
  inline
  double
  ComputeVolumeOfTetrahedron(const V& a, const V& b, const V& c, const V& d)
 ...
 */
inline double
SolveASinXPlusBCosXIsEqualToC(double a, double b, double c)
{
