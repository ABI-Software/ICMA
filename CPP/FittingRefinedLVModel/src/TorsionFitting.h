...
#ifndef TORSIONFITTING_H_
#define TORSIONFITTING_H_
...
class TorsionFitting {
	  ...
	  std::vector<std::vector<alglib::spline1dinterpolant> > nodeValues;
	  std::vector<std::vector<std::vector<double> > > torsionAngles;
	  const static int NUMBER_OF_NODES = 98;
	  std::vector<double> initialthetas;
	  int epiNodeIds[49];
	  int endoNodeIds[49];
public:
	TorsionFitting(std::vector<std::string> mesh, double focalLength);
	virtual ~TorsionFitting();
	void setUp(XMLInputReader& reader);
	std::vector<std::string> getMesh();
};

#endif /* TORSIONFITTING_H_ */
