...
#ifndef LVHEARTMESHMODEL_H_
#define LVHEARTMESHMODEL_H_
...
struct AxisFittingOptimizationInput{
	Cmiss_mesh_id mesh;
	Cmiss_field_module_id field_module;
	Cmiss_field_id coordinates_rc;
	Cmiss_field_cache_id cache;
	Cmiss_field_id transform_mx;
	std::vector<Cmiss_element_id>* cmiss_elements;
	Point3D targetApex;
	Point3D targetBasel;
	Point3D targetBaser;
	Point3D apexXi;
...
#endif
class LVHeartMeshModel {
public:
	//Default constructor
	LVHeartMeshModel(unsigned int numFrames);
	virtual ~LVHeartMeshModel();

	//Set the apex coordinate for the LV Mesh
	void setApex(Point3D apex);

	//Set the baseL coordinate for the LV Mesh
	void setBase(Point3D base);

	void setEDFrameData(std::vector<Point3D> bpts);
	...
	void setRVInserts(std::vector<Point3D> points);

	//Output the LV Meshes with filenames starting with prefix
	//in the given directory
	void outputMesh(std::string prefix, std::string directory);
	 ...
	 */
	std::vector<Point3D> getSpeckleInitializers(double time, int* apexIndex);

	//Fit LV Mesh to the input marker data
	void FitModel(std::vector<Point3D> points, double time, bool useBoundaryXi = false);

	/**
	 * Returns the normalized times for frame positions to be extracted
	 * that coincide with appropriate base plane variation
	 */
	std::vector<double> getFramesFromApexBaseLengthVariation();
	 ...
	 * @param	time [0,1]	The time of the model to use
	...
	Point3D computeCoordinates(double time, int elementid, Point3D& xi);

	std::vector<Point3D> getRefinedMeshCoordinates(double time);


	std::vector<Point3D> computePlaneBoundary(Point3D bl,Point3D br,double time,double wallxi=0.0);


protected:

	double getNearestMeshTime(double time);
	 ...
	 */
	void SetMuFromBasePlaneAtTime(Plane& basePlane, double time);
	 ...
	 */
	Plane FitPlaneToBasePlanePoints(const std::vector<std::pair<Point3D, double> >& basePlanePoints, const Vector3D& xAxis) const;
	 ...
	 * @param	frameNumber	The frame number.
	 */
	void UpdateTimeVaryingDataPoints(const cap::Vector& x, int frameNumber);
	...
	std::vector<double> ConvertToHermite(const cap::Vector& bezierParams);
	...
	std::vector<Point3D> edBasePlane;
