...
LVHeartMeshModel::LVHeartMeshModel(unsigned int numFrames, int type, unsigned int maxSpecklesPerSide) :
		focalLength(0.0), bezierToHermiteTransform_(NULL), field_(0), field_module_(0), coordinates_ps_(0), coordinates_rc_(0), coordinates_patient_rc_(0), transform_mx_(0), numberOfModelFrames_(
				numFrames), viewType(type), MAXSPECKLESPERSIDE(maxSpecklesPerSide), aligned(0), targetImageWidth(800), targetImageHeight(600), timeVaryingDataPoints_(134), solverFactory_(
				new cap::GMMFactory), timeSmoother_() {
...
void LVHeartMeshModel::setBaseL(Point3D base) {
	this->basel = base;
}

void LVHeartMeshModel::setBaseR(Point3D base) {
	this->baser = base;
}
...
void LVHeartMeshModel::SetMuFromBasePlaneAtTime(const Plane& basePlane, double time) {
...
#else
	// EPI nodes [0-19], ENDO nodes [20-39], node identifiers are base 1.
	// Similar to cap method but EPI nodes are skipped
	// Their mu is set to that of their corresponding endo node
...
void LVHeartMeshModel::AlignModel() {
	...
	if (!aligned) {
		...
		//to align them
		{
			...
			//http://stackoverflow.com/questions/3780493/map-points-between-two-triangles-in-3d-space
...
std::vector<Point3D> LVHeartMeshModel::getSpeckleInitializers(double time, int* apexIndex) {
	double temp[3], coord[3];
	Cmiss_field_cache_set_time(cache, time);
...
double LVHeartMeshModel::getApexBaseLengthRatioAt(int frame) {
...
Point3D LVHeartMeshModel::getBaseL() {
	return basel;
}

Point3D LVHeartMeshModel::getBaseR() {
	return baser;
}
...
void LVHeartMeshModel::FitModel(std::vector<Point3D> points, double timex, bool useBoundaryXi) {
	double time = timex;
	double denom = numberOfModelFrames_ - 1;
	for (unsigned int i = 0; i < numberOfModelFrames_; i++) {
		double ttime = static_cast<double>(i) / denom;
		if (ttime <= timex) {
			time = ttime;
		} else
			break;
	...
	if (numPoints > 0) {
		...
		if (useBoundaryXi && speckleXi.size() == points.size()) { //If the number of points do no match then find the xi
			xivec = &speckleXi;
		...
		} catch (gmm::gmm_error& err) {
			std::cout << "Fitting error occured for " << std::endl;
			std::cout << "Detected Coordinates\t PS coordinates\t Xi coordinates\t Element ID" << std::endl;
			for (int i = 0; i < points.size(); i++) {
				std::cout << points[i] << "\t" << pscoord[i] << "\t" << xi[i] << "\t" << elem_id[i] << std::endl;
			}
			std::cout << "Speckle xi size " << xi.size() << "\t points size " << points.size() << std::endl;
