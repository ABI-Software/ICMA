...
public:
	//Default constructor
	LVHeartMeshModel(unsigned int numFrames, int view, unsigned int maxSpecklesPerSide);
	virtual
	~LVHeartMeshModel();

	void setTargetImageDimensions(int width,int height){
	 ...
	 */
	std::vector<Point3D>
	getSpeckleInitializers(double time, int* apexIndex);
...
protected:
	...
	double focalLength;
	bool aligned;
	unsigned int targetImageWidth;
	unsigned int targetImageHeight;
	int baselElem;
	int baserElem;
