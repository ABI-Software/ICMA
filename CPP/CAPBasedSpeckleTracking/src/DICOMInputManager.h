...
typedef itk::Image<double, 2> DICOMSliceImageType;
typedef itk::Image<double, 3> Image3D;

typedef itk::RGBToLuminanceImageFilter<InputImageType, Image3D> RGBToLuminanceImageFilterType;
typedef itk::ExtractImageFilter<Image3D, DICOMSliceImageType> ExtractImageFilterType;
...
class DICOMInputManager {
	...
	int NUMCAPFRAMES; //Defaults to 9
...
public:
	...
	std::vector<DICOMSliceImageType::Pointer>* extractEDToECFrames();
	std::vector<DICOMSliceImageType::Pointer>* extractSpecifiedFrames(std::vector<double> iw);
	/**
	...
	inline int getPulseLength();
        ...
        /**
         * The duration of a cardiac cycle (rwave-rwave) in milli seconds
         * @return is a positive number if available, else returns -1 (esp. if ed and ec are set)
         */
	...
	InputImageType::Pointer getTargetDicom();
