...
typedef itk::Image<unsigned int, 2> DICOMSliceImageType;
typedef itk::Image<double, 2> IntensityImageType;
typedef itk::Image<PixelType, 2> RGBImageType;

typedef itk::ImageFileWriter<RGBImageType> Writer2DType;
...
typedef itk::IntensityWindowingImageFilter <DICOMSliceImageType, IntensityImageType> IntensityWindowingImageFilterType;
typedef itk::RescaleIntensityImageFilter< DICOMSliceImageType, IntensityImageType > RescaleFilterType;
typedef itk::ScalarToRGBColormapImageFilter<IntensityImageType, RGBImageType> RGBFilterType;
...
class DICOMInputManager {
	...
	std::vector<unsigned int> windowWidth;
	std::vector<unsigned int> windowCenter;
	std::vector<std::string>  positionPatient;
	std::vector<std::string>  orientationPatient;
	std::vector<std::string>  instanceId;
	std::string workingDir; //The working director where WADO stream is stored
	std::vector<std::string> inputFile; //The input file for itk file reader filter
...
public:
	 ...
	 */
	DICOMInputManager(DCMTKUtils& DCMtkutil, std::vector<std::string> filename, int numframes = -1);
	/**
	 * Simple constructor that takes a file or a url to a service from which the input DICOM can be
	 * accessed. If the input parameter begins with http, the input is considered as a url and a http
	 * or https request is initiated when Initialize is called.
	 ...
	 * @param unsigned int, the number of frames to be extracted (-1) all frames
	 */
	DICOMInputManager(DCMTKUtils& DCMtkutil,std::vector<std::string> filename, std::string dir,
			int numframes = -1);
