...
public class MRIFitData {
	...
	private String modelEDtime = null;
	private String modelEStime = null;
	...
	private Hashtable<String, String> coordinates; // View, transformation
													// matrix map
	private Hashtable<String, String> viewPlaneMap;
	private Hashtable<String, Hashtable<Double, String[]>> strains; // Map of
																	...
																	// wall
																	// strains
	private Hashtable<String, String> strainGroup; // Map of strainName to
													// strainGroup
	...
	public String getSopintanceUID(){
		return sopinstanceUID;
	}
	
	public Properties getViewUrls(){
		Properties result = new Properties();
		JSONArray jsonArray = (JSONArray) mriViewdata.get("planes");
		if (jsonArray != null) {
			int len = jsonArray.size();
			for (int i = 0; i < len; i++) {
				JSONObject planes = (JSONObject) jsonArray.get(i);
				String seriesid = (String) planes.get("seriesid");
				String type = viewPlaneMap.get(seriesid);
				if(type==null)
					type = "UN";
				try{
					String vname = type+"_"+i;
					int numImages = Integer.parseInt((String) planes.get("NUMFRAMES"));
					JSONObject frames = (JSONObject) planes.get("frames");
					Properties viewurls = new Properties();
					//Get the list of urls
					for (int j = 0; j < numImages; j++) {
						viewurls.put(vname+"_"+j,(String)frames.get(""+j));
					}
					result.put(vname, viewurls);
				}catch(Exception exx){
	...
	private void parseXML() throws Exception {
		...
		// Load the modeledtime
		nList = doc.getElementsByTagName("MODELEDTIME");
		for (int i = 0; i < nList.getLength(); i++) {
			Node nNode = nList.item(i);
			NodeList text = nNode.getChildNodes();
			modelEDtime = ((Node) text.item(0)).getNodeValue().trim();
		}

		// Load the modelestime
		nList = doc.getElementsByTagName("MODELESTIME");
		for (int i = 0; i < nList.getLength(); i++) {
			Node nNode = nList.item(i);
			NodeList text = nNode.getChildNodes();
			modelEStime = ((Node) text.item(0)).getNodeValue().trim();
		}

		// Load the targetview serial id
		nList = doc.getElementsByTagName("TARGETVIEW");
		for (int i = 0; i < nList.getLength(); i++) {
			Node nNode = nList.item(i);
			NodeList text = nNode.getChildNodes();
			targetView = ((Node) text.item(0)).getNodeValue().trim();
		}
		
		// Load the targetview serial id
		nList = doc.getElementsByTagName("SOPINSTANCEUID");
		for (int i = 0; i < nList.getLength(); i++) {
			Node nNode = nList.item(i);
			NodeList text = nNode.getChildNodes();
			sopinstanceUID = ((Node) text.item(0)).getNodeValue().trim()+Math.random();//To avoid same targetview uid clashes
		}		
		...
		//Load view to plane type mapping
		nList = doc.getElementsByTagName("VIEWPLANETYPE");
		for (int i = 0; i < nList.getLength(); i++) {
			Node nNode = nList.item(i);
			NodeList text = nNode.getChildNodes();
			String content = ((Node) text.item(0)).getNodeValue().trim();
			...
			for (int j = 0; j < attrs.getLength(); j++) {
				Attr attribute = (Attr) attrs.item(j);
				if (attribute.getName().trim().equalsIgnoreCase("VIEW")) {
					seriesid = attribute.getValue().trim();
				}
			}
			if (seriesid!=null) {
				viewPlaneMap.put(seriesid, content);
			}
		}
	...
	public String getPatientID(){
		return (String)mriViewdata.get("patientID");
	}

	public String getStudyUID(){
		return (String)mriViewdata.get("studyUID");
	}
	
	public String getJSON(Properties exXpaths) {
		JSONObject model = new JSONObject();
		model.put("type", "MR");
		model.put("studyUid", getStudyUID());
		model.put("name", modelName);
		model.put("startTime", 0);
		model.put("endTime", duration);
		model.put("ed", modelEDtime);
		model.put("es", modelEStime);
		model.put("targetView", targetView);
		...
		//The view information is available in the MRI view object
		//Update it with plane type
		JSONArray jsonArray = (JSONArray) mriViewdata.get("planes");
		if (jsonArray != null) {
			int len = jsonArray.size();
			for (int i = 0; i < len; i++) {
				...
				if(type==null)
					type = "UN";
				planes.put("type", type);
				display.put(type+""+i, "surface");
				iregions.put(type+""+i, planes);
			}
		}
	...
	public static String getFittingXML(ByteArrayInputStream dicomFile) throws Exception {
		...
		// Load the meshes (Note there could be more than meshes - but each mesh
		// tag has a heart/organ)
		NodeList nList = doc.getElementsByTagName("CMISSCAPIO");
	...
	public static void main(String args[]) throws Exception {
		...
		MRIFitData metaData = new MRIFitData("/home/rjag008/ClevelandClinic/CPP/MRIModelStrainMeasures/Release/strain.xml", new JSONObject());
