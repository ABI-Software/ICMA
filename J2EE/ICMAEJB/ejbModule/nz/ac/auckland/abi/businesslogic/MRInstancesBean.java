 ...
 */
@Stateless
public class MRInstancesBean implements MRInstancesBeanRemote {
     ...
     * Default constructor. 
     */
    public MRInstancesBean() {
       
    }

    
	public void addInstance(MRStudyInstances instance){
		entityManager.persist(instance);
	}
	
	public void removeInstance(MRStudyInstances instance){
		entityManager.remove(entityManager.merge(instance)); //To handle detached instances, merge them with em
	}
	
	public List<MRStudyInstances> getInstancesForStudy(PACSStudy study){
	   	String q = "SELECT p from " + MRStudyInstances.class.getName() + " p where p.studyID = :study_id";
        Query query = entityManager.createQuery(q).setParameter("study_id",study.getStudyID());
        List<MRStudyInstances> instances = query.getResultList();
        return instances;
	}
}
